...
const BeforeAfterSlider = ({ beforeUrl, afterUrl }: { beforeUrl: string; afterUrl: string }) => {
  ...
  return (
    <div className="relative w-full aspect-[4/3] rounded-lg overflow-hidden select-none">
      {/* After image (full) */}
      <img src={afterUrl} alt="Sonra" className="absolute inset-0 w-full h-full object-cover" />
      
      {/* Before image (clipped) */}
      <div className="absolute inset-0 overflow-hidden" style={{ width: `${position}%` }}>
        <img src={beforeUrl} alt="Önce" className="absolute inset-0 w-full h-full object-cover" style={{ minWidth: `${100 / (position / 100)}%`, maxWidth: `${100 / (position / 100)}%` }} />
      </div>
      ...
      {/* Labels */}
      <div className="absolute top-3 left-3 bg-surface-dark/80 text-surface-dark-foreground px-3 py-1 rounded text-xs font-bold">ÖNCE</div>
      <div className="absolute top-3 right-3 bg-brand/90 text-brand-foreground px-3 py-1 rounded text-xs font-bold">SONRA</div>
...
const GallerySection = () => {
  return (
    <section className="py-16 md:py-24 bg-muted/50" id="galeri">
      <div className="container">
        <div className="text-center mb-12">
          <span className="text-brand font-bold text-sm uppercase tracking-widest">Referanslarımız</span>
          <h2 className="text-3xl md:text-4xl font-black text-foreground mt-2">İş Galerimiz</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
            ...
            />
            <p className="text-center text-muted-foreground text-sm mt-3 font-medium">Banyo Renovasyonu</p>
          </motion.div>
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.15 }}>
            ...
            />
            <p className="text-center text-muted-foreground text-sm mt-3 font-medium">Mutfak Yenileme</p>
          </motion.div>
        </div>
